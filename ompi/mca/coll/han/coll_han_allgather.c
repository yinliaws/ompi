...
{
    *item = NULL;
    if (frag_size > 0 && needed <= frag_size) {
    ...
    if (!t->noop) {
        ...
        if (mca_coll_han_component.han_use_persist_buffers && t->han_module != NULL) {
            if ((size_t)rsize <= mca_coll_han_component.han_fragment_size
                ...
                if (t->han_module->allgather_gather_persist.size < (size_t)rsize) {
                    char *p = realloc(t->han_module->allgather_gather_persist.buf, rsize);
                    if (NULL == p) return OMPI_ERR_OUT_OF_RESOURCE;
                    t->han_module->allgather_gather_persist.buf = p;
                    t->han_module->allgather_gather_persist.size = rsize;
                }
                tmp_buf = t->han_module->allgather_gather_persist.buf;
            }
        } else {
        ...
        if (t->is_mapbycore) {
            ...
            rsize =
                opal_datatype_span(&t->rdtype->super,
                                   ...
                                   &rgap);
            if (mca_coll_han_component.han_use_persist_buffers && t->han_module != NULL) {
                if (t->han_module->allgather_reorder_persist.size < (size_t)rsize) {
                    char *p = realloc(t->han_module->allgather_reorder_persist.buf, rsize);
                    if (NULL == p) return OMPI_ERR_OUT_OF_RESOURCE;
                    t->han_module->allgather_reorder_persist.buf = p;
                    t->han_module->allgather_reorder_persist.size = rsize;
                }
                reorder_buf = t->han_module->allgather_reorder_persist.buf;
            } else {
...
allgather_done:
    ;  /* empty statement after label (C99) */
...
#define HAN_MIN_PIPELINE_FRAGS 4
    ...
    if (frag_size == 0 || gathered_size <= (HAN_MIN_PIPELINE_FRAGS * frag_size)) {
        ...
        /* Non-mapbycore path: packetized pipeline using non-blocking igather+ibcast
         * on up_comm, with blocking low_comm gather as the sync point.
         * Allgather decomposed as: igather to root leader + ibcast from root.
         * Pattern follows allreduce: non-blocking inter-node ops overlap with
         * the next intra-node gather.
         */

        /* Single-fragment path: gather into reorder_buf, in-place allgather,
         * reorder, bcast. Use freelist only when total size >= 64KB. */
        if (num_frags == 1) {
            char *reorder_buf = NULL;
            char *reorder_buf_start = NULL;
            char *my_slot = NULL;
        ...
        /* Pipeline path with double-buffered frag_reorder.
         * Two reorder buffers allow igather[N] to write buf_cur while
         * we ibcast+reorder from buf_prev, maximizing overlap.
         *
         * Steady-state per fragment:
         *   1. Wait igather[N-1] (into buf_prev)
         *   2. Start ibcast[N-1] from buf_prev (non-blocking)
         *   3. Blocking gather[N] on low_comm (all ranks)
         *   4. Start igather[N] into buf_cur (non-blocking)
         *   5. Wait ibcast[N-1], reorder buf_prev into rbuf
         *      (overlaps with igather[N] in flight)
         *   6. Swap buf_cur <-> buf_prev
         */
        char *frag_reorder[2] = {NULL, NULL};
        opal_free_list_item_t *frag_reorder_item[2] = {NULL, NULL};
        int frag_reorder_src[2] = {0, 0};
        if (low_rank == root_low_rank) {
        ...
        for (size_t frag = 0; frag < num_frags; frag++) {
            ...
            if (frag > 0 && low_rank == root_low_rank) {
                /* Wait for previous igather (wrote into frag_reorder[prev_buf]) */
                ompi_request_wait(&igather_req, MPI_STATUS_IGNORE);
                igather_req = NULL;

                /* Return previous gather buffer */
                han_free_frag(&han_module->fragment_freelist,
                              inter_frag_item, gather_buf);
                inter_frag_item = NULL;
                gather_buf = NULL;

                /* Start ibcast of prev frag from frag_reorder[prev_buf] */
            ...
            /* Allocate gather buffer for this fragment from freelist */
            if (low_rank == root_low_rank) {
            ...
            /* Leaders: igather into frag_reorder[cur_buf] — separate from
             * prev_buf so ibcast[N-1] and igather[N] don't conflict */
            if (low_rank == root_low_rank) {
            ...
            /* Wait for ibcast of prev frag, then reorder into rbuf.
             * igather[N] is in flight concurrently on frag_reorder[cur_buf]. */
            if (frag > 0 && low_rank == root_low_rank) {
                ...
                for (int i = 0; i < up_size; i++) {
                    for (int j = 0; j < low_size; j++) {
                        ...
                        ompi_datatype_copy_content_same_ddt(rdtype,
                            (ptrdiff_t)prev_frag_count,
                            (char *)rbuf + rextent * ((ptrdiff_t)dest_rank * (ptrdiff_t)rcount
                                                      + (ptrdiff_t)prev_frag_offset),
                            frag_reorder[prev_buf] + rextent * (ptrdiff_t)global_idx
                                         * (ptrdiff_t)prev_frag_count);
                    }
                }
            }
        ...
        /* Epilogue: last frag is in frag_reorder[1 - cur_buf] */
        if (low_rank == root_low_rank) {
            int last_buf = 1 - cur_buf;

            /* Wait for last igather */
            if (igather_req != NULL) {
                ompi_request_wait(&igather_req, MPI_STATUS_IGNORE);
            }

            /* Return last gather buffer */
            han_free_frag(&han_module->fragment_freelist,
                          inter_frag_item, gather_buf);

            /* ibcast last frag from frag_reorder[last_buf] */
            ...
            /* Reorder last fragment into rbuf */
            for (int i = 0; i < up_size; i++) {
                for (int j = 0; j < low_size; j++) {
                    int global_idx = i * low_size + j;
                    int dest_rank = topo[global_idx * 2 + 1];
                    ompi_datatype_copy_content_same_ddt(rdtype,
                        ...
                        (char *)rbuf + rextent * ((ptrdiff_t)dest_rank * (ptrdiff_t)rcount
                                                  + (ptrdiff_t)prev_frag_offset),
                        frag_reorder[last_buf] + rextent * (ptrdiff_t)(i * low_size + j)
                                     * (ptrdiff_t)prev_frag_count);
                }
            }

            /* Free both reorder buffers */
            for (int b = 0; b < 2; b++) {
        ...
        /* Final low_comm bcast of complete result */
        low_comm->c_coll->coll_bcast(rbuf, rcount * low_size * up_size, rdtype,
                                     root_low_rank, low_comm,
                                     low_comm->c_coll->coll_bcast_module);
    }
